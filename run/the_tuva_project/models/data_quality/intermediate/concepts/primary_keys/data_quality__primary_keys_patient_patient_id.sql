        ...
        create or replace transient table tuva_synthetic.data_quality.primary_keys_patient_patient_id
         as
        (
...
WITH valid_conditions AS (
    SELECT
        *
    FROM
        tuva_synthetic.data_quality.patient_patient_id
    WHERE
        bucket_name = 'valid'
)
, uniqueness_check as (
...
, duplicates_summary AS (
    SELECT
        ...
        row_number() over (order by drill_down_key) as row_number_value
    FROM
        tuva_synthetic.data_quality.patient_patient_id a
    JOIN
