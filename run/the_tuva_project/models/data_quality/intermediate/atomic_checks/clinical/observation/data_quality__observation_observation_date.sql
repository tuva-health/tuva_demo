        ...
        create or replace transient table tuva_synthetic.data_quality.observation_observation_date
         as
        (
...
SELECT
    ...
    , case
        when m.observation_date > cast(substring('2024-10-04 19:08:24.316902+00:00',1,10) as date) then 'invalid'
        when m.observation_date <= cast('1901-01-01' as date) then 'invalid'
        when m.observation_date is null then 'null'
        else 'valid'
    end as bucket_name
    , case
        when m.observation_date > cast(substring('2024-10-04 19:08:24.316902+00:00',1,10) as date) then 'future'
